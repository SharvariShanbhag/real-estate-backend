...
const submitInquiry = async (req, res) => {
    try {
        ...
        // --- CHANGE THIS LINE ---
        if (!name || !email || !message) { // Removed !phone from this check
            return res.status(400).json({
                success: false,
                error: "Name, email, and message are required", // Updated error message
            });
        }
        // --- END CHANGE ---
        ...
        const inquiry = await Inquiry.create({
            name,
            email,
            phone, // Will now accept '' or null from frontend
            message,
            propertyId: propertyId || null,
            userId: userId,
            inquiryType: propertyId ? 'Property-Specific' : 'General',
